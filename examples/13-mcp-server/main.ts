...
import { t, Result } from "../../packages/lfts-type-runtime/mod.ts";
import {
  createStageCatalog,
  defineBackendFunctionStage,
  defineFullStackHtmxStage,
} from "../../packages/lfts-type-runtime/stage-types.ts";
...
const processPaymentStage = defineBackendFunctionStage({
  ...
  description:
    "Process customer payment with fraud detection, authorization, and settlement. Automatically retries on network failures.",
...
const refundPaymentStage = defineBackendFunctionStage({
  ...
  description:
    "Refund a previous payment transaction. Validates original transaction exists and amount is valid. Creates audit trail.",
...
if (paymentStagesResult.ok) {
  const stages = paymentStagesResult.value as Array<{ name: string }>;
...
if (backendResult.ok) {
  const stages = backendResult.value as Array<{ name: string }>;
  console.log(`   Found ${stages.length} stages:`);
  for (const stage of stages) {
    console.log(`   - ${stage.name}`);
  }
