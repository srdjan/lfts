 ...
 * @module
 */
import { generateJsonSchema } from "../lfts-codegen/json-schema.js";
/**
 * Convert a StageDefinition to an MCP tool definition
 */
export function stageToMcpTool(stage, options = {}) {
 ...
 */
function formatToolName(stageName) {
    return stageName
        .replace(/([A-Z])/g, "_$1")
        .toLowerCase()
        .replace(/^_/, "");
 ...
 */
function buildToolDescription(stage, options) {
    const parts = [];
    // Main description
    ...
    if (options.includeRetryInfo && stage.metadata?.retry) {
        ...
        if (stage.metadata.retry.backoffMultiplier) {
            parts.push(`- Backoff multiplier: ${stage.metadata.retry.backoffMultiplier}`);
        }
    ...
    if (stage.metadata?.stageKind === "backend_function") {
        const backendMeta = stage.metadata;
 ...
 */
export function stagesToMcpTools(stages, options = {}) {
