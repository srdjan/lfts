// packages/lfp-type-runtime/result-option-examples.ts
// Comprehensive examples for Result/Option combinators (Phase 1.1)

import { Result, Option, validate, type ValidationError } from "./mod.ts";
...
const getValidPort = (config: Config): Option<number> => {
  return Option.andThen(Option.from(config.port), (p) =>
    p > 0 && p < 65536 ? Option.some(p) : Option.none()
  );
};
...
console.log("Full config:", buildConnectionString({ host: "localhost", port: 8080 }));
