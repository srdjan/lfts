...
console.log("\nTest 7: String minLength and maxLength combined");
const passwordSchema = enc.refine.minLength(enc.refine.maxLength(enc.str(), 20), 8);
...
console.log("\nTest 10: Array minItems and maxItems combined");
const teamSchema = enc.refine.minItems(enc.refine.maxItems(enc.arr(enc.str()), 10), 3);
...
const userSchema = enc.obj([
  { name: "username", type: enc.refine.minLength(enc.refine.maxLength(enc.str(), 20), 3) },
...
console.log("\nTest 14: Refinements with READONLY and BRAND wrappers");
const positiveIntSchema = enc.brand(enc.refine.min(enc.refine.integer(enc.num()), 1), "PositiveInt");
