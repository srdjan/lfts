...
import { Result } from "../../../../../../lfp-type-runtime/mod.ts";
...
export const mapExample = (r: Result<number, string>): Result<string, string> => {
  return Result.map(r, (n) => `Result: ${n}`);
};

export const andThenExample = (r: Result<number, string>): Result<number, string> => {
