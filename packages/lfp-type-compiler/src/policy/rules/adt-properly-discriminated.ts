...
function resolveToUnionTypeNode(node: ts.TypeNode, checker: ts.TypeChecker): ts.UnionTypeNode | null {
...
export const adtProperlyDiscriminatedRule: Rule = {
  ...
  analyzeUsage(node, ctx) {
    ...
    for (const alt of u.types) {
      ...
      if (!found) {
        ctx.report(u, `LFP1006: ADT must be discriminated by a '${TAG}' string-literal property with unique string literal values.`);
        return;
      }
