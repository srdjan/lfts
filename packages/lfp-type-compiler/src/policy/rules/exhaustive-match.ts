...
export const exhaustiveMatchRule: Rule = {
  meta: {
    ...
    defaultOptions: {},
    description: "match(value, cases) must handle all ADT variants (discriminated by 'type') with no extras.",
  },
  analyzeUsage(node, ctx) {
    if (!ts.isCallExpression(node)) return;
    if (!ts.isIdentifier(node.expression) || node.expression.text !== "match") return;
    ...
    for (const prop of casesExpr.properties) {
      ...
      const key = ts.isIdentifier(name) || ts.isStringLiteral(name) || ts.isNumericLiteral(name) ? (name as any).text : undefined;
      if (!key) continue;
      if (expected.has(key)) {
        provided.push(key);
      } else {
    ...
    if (missing.length || extra.length) {
      ...
      if (missing.length) messages.push(`missing cases: ${missing.map(x => `'${x}'`).join(", ")}`);
      if (extra.length) messages.push(`extra cases: ${extra.map(x => `'${x}'`).join(", ")}`);
      ctx.report(casesExpr, `LFP1007: non-exhaustive match over ADT (${messages.join("; ")})`);
    }
  }
};
