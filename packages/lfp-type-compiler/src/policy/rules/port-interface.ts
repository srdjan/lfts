// packages/lfp-type-compiler/src/policy/rules/port-interface.ts
...
export const portInterfaceRule: Rule = {
  meta: {
    ...
    defaultOptions: {},
    description: "Interfaces used as capabilities must contain only function members.",
  },
  analyzeDeclaration(node, ctx) {
    ...
    for (const prop of ty.getProperties()) {
      ...
      if (pT.getCallSignatures().length === 0) {
        ctx.report(node, `LFP1001: Interface '${name}' must only contain function members. Non-function property: '${prop.getName()}'.`);
      }
    }
  }
};
