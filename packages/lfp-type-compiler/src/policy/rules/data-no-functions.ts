    ...
    while (stack.length) {
      ...
      for (const p of t.getProperties()) {
        ...
        const isBuiltin = declarations.some(decl => {
          ...
          return sourceFile.fileName.includes('lib.') && sourceFile.fileName.endsWith('.d.ts');
        });
        ...
        if (pT.getCallSignatures().length > 0) {
          ctx.report(node, `LFP1003: Data schema contains function property '${propName}'. Move behavior into a capability interface.`);
          return;
        }
        stack.push(pT);
      }
