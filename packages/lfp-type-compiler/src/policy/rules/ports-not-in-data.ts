// packages/lfp-type-compiler/src/policy/rules/ports-not-in-data.ts
  ...
  analyzeUsage(node, ctx) {
    ...
    while (stack.length) {
      ...
      if (sym && ctx.classify.isPort(sym)) {
        ctx.report(node, `LFP1002: Capability '${sym.getName()}' must not appear in data schemas (typeOf<T>()). Inject via parameters instead.`);
        return;
      }
