// packages/lfts-type-runtime/benchmark-union.ts
...
function benchmark(name: string, fn: () => void): number {
  ...
  console.log(
    `${name}: ${totalMs.toFixed(2)} ms (${opsPerSec.toLocaleString()} ops/sec)`,
  );
  return opsPerSec;
}
...
for (const variantCount of [2, 5, 10]) {
  ...
  console.log(
    `  → ${
      (1000000 / (ITERATIONS / (performance.now() - performance.now() + 1)))
        .toFixed(3)
    } μs per validation`,
  );
}
