// packages/lfts-type-runtime/benchmark.ts
// Benchmark suite for LFTS runtime validator performance
...
function printComparison(baseline: BenchResult, optimized: BenchResult) {
  ...
  console.log(
    `  ${optimized.name} is ${
      speedup.toFixed(2)
    }x faster (${improvement}% improvement)`,
  );
  console.log(
    `  ${baseline.opsPerSec.toLocaleString()} → ${optimized.opsPerSec.toLocaleString()} ops/sec`,
  );
}
