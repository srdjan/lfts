...
// Covers both structural introspection and runtime hooks
...
Deno.test("introspection hooks: basic success hook", () => {
  const schema = enc.obj([
...
Deno.test("introspection hooks: basic failure hook", () => {
  const schema = enc.obj([
    { name: "name", type: enc.str() },
    { name: "age", type: enc.num() },
  ]);
...
Deno.test("introspection hooks: metadata extraction", () => {
  const schema = enc.obj([
...
Deno.test("introspection hooks: multiple hooks can be registered", () => {
  const schema = enc.str();
...
Deno.test("introspection hooks: validateUnsafe triggers hooks", () => {
  const schema = enc.num();
...
Deno.test("introspection hooks: validateAll triggers hooks", () => {
  const schema = enc.obj([
    { name: "a", type: enc.num() },
    { name: "b", type: enc.num() },
  ]);
...
Deno.test("introspection hooks: hooks don't break validation on error", () => {
  const schema = enc.str();
...
Deno.test("introspection hooks: practical debugging example", () => {
  type Order = {
    id: string;
    total: number;
  };
