...
console.log(
  `  READONLY wrapper:      ${readonlyOps.toLocaleString()} ops/sec (${
    ((readonlyOps / baselineOps) * 100).toFixed(1)
  }% of baseline)`,
);
console.log(
  `  BRAND wrapper:         ${brandOps.toLocaleString()} ops/sec (${
    ((brandOps / baselineOps) * 100).toFixed(1)
  }% of baseline)`,
);
console.log(
  `  Nested READONLY:       ${nestedOps.toLocaleString()} ops/sec (${
    ((nestedOps / baselineOps) * 100).toFixed(1)
  }% of baseline)`,
);
