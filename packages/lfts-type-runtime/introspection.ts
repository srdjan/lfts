...
/**
 * Property metadata from an OBJECT schema
 */
...
/**
 * Variant metadata from a DUNION schema
 */
...
/**
 * Refinement constraint metadata
 */
...
export type SchemaInfo =
  | { readonly kind: "primitive"; readonly type: "string" | "number" | "boolean" | "null" | "undefined" }
  | { readonly kind: "literal"; readonly value: string | number | boolean }
  | { readonly kind: "array"; readonly element: TypeObject }
  | { readonly kind: "tuple"; readonly elements: readonly TypeObject[] }
  | { readonly kind: "object"; readonly properties: readonly PropertyInfo[]; readonly strict: boolean }
  | { readonly kind: "union"; readonly alternatives: readonly TypeObject[] }
  | { readonly kind: "dunion"; readonly discriminant: string; readonly variants: readonly VariantInfo[] }
  | { readonly kind: "brand"; readonly tag: string; readonly inner: TypeObject }
  | { readonly kind: "readonly"; readonly inner: TypeObject }
  | { readonly kind: "refinement"; readonly refinements: readonly RefinementInfo[]; readonly inner: TypeObject }
  | { readonly kind: "result"; readonly valueType: TypeObject; readonly errorType: TypeObject | null }
  | { readonly kind: "option"; readonly valueType: TypeObject | null }
  | { readonly kind: "metadata"; readonly metadata: { name?: string; source?: string }; readonly inner: TypeObject }
  | { readonly kind: "port"; readonly portName: string; readonly methods: readonly PortMethodInfo[] }
  | { readonly kind: "effect"; readonly effectType: string; readonly returnType: TypeObject };

/**
 * Port method metadata
 */
export type PortMethodInfo = {
  readonly name: string;
  readonly params: readonly TypeObject[];
  readonly returnType: TypeObject;
};
...
export function introspect(schema: TypeObject): SchemaInfo {
  ...
  switch (op) {
    ...
    // Literal
    case Op.LITERAL:
      return { kind: "literal", value: bc[1] as string | number | boolean };

    // Array
    case Op.ARRAY:
      return { kind: "array", element: bc[1] as TypeObject };

    // Tuple
    case Op.TUPLE: {
      const length = bc[1] as number;
      const elements: TypeObject[] = [];
      for (let i = 0; i < length; i++) {
        elements.push(bc[2 + i] as TypeObject);
      }
    ...
    case Op.OBJECT: {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const name = bc[idx++] as string;
        const optional = bc[idx++] === 1;
        const type = bc[idx++] as TypeObject;
    ...
    case Op.UNION: {
      const count = bc[1] as number;
      const alternatives: TypeObject[] = [];
      for (let i = 0; i < count; i++) {
        alternatives.push(bc[2 + i] as TypeObject);
      }
    ...
    case Op.DUNION: {
      ...
      const count = bc[2] as number;
      const variants: VariantInfo[] = [];
      for (let i = 0; i < count; i++) {
        const tag = bc[3 + 2 * i] as string;
        const schema = bc[3 + 2 * i + 1] as TypeObject;
    ...
    // Readonly
    case Op.READONLY:
      return { kind: "readonly", inner: bc[1] as TypeObject };

    // Brand
    case Op.BRAND:
      return { kind: "brand", tag: bc[1] as string, inner: bc[2] as TypeObject };
    ...
    case Op.REFINE_PATTERN: {
      ...
      let current = bc;
      let innerSchema: TypeObject = bc;

      while (true) {
        const currentOp = current[0] as Op;

        switch (currentOp) {
          case Op.REFINE_MIN:
            refinements.push({ kind: "min", value: current[1] as number });
            innerSchema = current[2] as TypeObject;
            current = innerSchema as any[];
            break;
          case Op.REFINE_MAX:
            refinements.push({ kind: "max", value: current[1] as number });
            innerSchema = current[2] as TypeObject;
            current = innerSchema as any[];
            break;
          case Op.REFINE_INTEGER:
            ...
            break;
          case Op.REFINE_MIN_LENGTH:
            refinements.push({ kind: "minLength", value: current[1] as number });
            innerSchema = current[2] as TypeObject;
            current = innerSchema as any[];
            break;
          case Op.REFINE_MAX_LENGTH:
            refinements.push({ kind: "maxLength", value: current[1] as number });
            innerSchema = current[2] as TypeObject;
            current = innerSchema as any[];
            break;
          case Op.REFINE_MIN_ITEMS:
            refinements.push({ kind: "minItems", value: current[1] as number });
            innerSchema = current[2] as TypeObject;
            current = innerSchema as any[];
            break;
          case Op.REFINE_MAX_ITEMS:
            refinements.push({ kind: "maxItems", value: current[1] as number });
            innerSchema = current[2] as TypeObject;
            current = innerSchema as any[];
            break;
          case Op.REFINE_EMAIL:
    ...
    case Op.RESULT_OK:
      return {
        kind: "result",
        valueType: bc[1] as TypeObject,
        errorType: null,
      };
    case Op.RESULT_ERR:
      return {
        kind: "result",
        valueType: null as any, // Not present in ERR variant
        errorType: bc[1] as TypeObject,
      };
    ...
    case Op.METADATA:
      return {
        kind: "metadata",
        metadata: bc[1] as { name?: string; source?: string },
        inner: bc[2] as TypeObject,
      };

    // Port
    case Op.PORT: {
      const portName = bc[1] as string;
      const methodCount = bc[2] as number;
      ...
      for (let i = 0; i < methodCount; i++) {
        ...
        const name = bc[offset + 1] as string;
        const paramCount = bc[offset + 2] as number;
        const params: TypeObject[] = [];
        for (let j = 0; j < paramCount; j++) {
          params.push(bc[offset + 3 + j] as TypeObject);
        }
        const returnType = bc[offset + 3 + paramCount] as TypeObject;
    ...
    case Op.EFFECT:
      return {
        kind: "effect",
        effectType: bc[1] as string,
        returnType: bc[2] as TypeObject,
      };
 ...
 */
export function getKind(schema: TypeObject): SchemaInfo["kind"] {
  assertBytecode(schema);
  const bc = schema as any[];
  const op = bc[0] as Op;
 ...
 */
export function getProperties(schema: TypeObject): readonly PropertyInfo[] {
 ...
 */
export function getVariants(schema: TypeObject): readonly VariantInfo[] {
  const info = introspect(schema);
 ...
 */
export function getRefinements(schema: TypeObject): readonly RefinementInfo[] {
  const info = introspect(schema);
 ...
 */
export function unwrapBrand(schema: TypeObject): TypeObject {
  const info = introspect(schema);
 ...
 */
export function unwrapReadonly(schema: TypeObject): TypeObject {
  const info = introspect(schema);
 ...
 * ```
 */
export function unwrapAll(schema: TypeObject): TypeObject {
  let current = schema;
...
/**
 * Visitor interface for traversing schema trees.
 * Implement only the methods you need; others default to identity.
 */
export interface SchemaVisitor<R> {
  primitive?: (type: "string" | "number" | "boolean" | "null" | "undefined") => R;
  literal?: (value: string | number | boolean) => R;
  array?: (element: TypeObject) => R;
  tuple?: (elements: readonly TypeObject[]) => R;
  object?: (properties: readonly PropertyInfo[], strict: boolean) => R;
  union?: (alternatives: readonly TypeObject[]) => R;
  dunion?: (discriminant: string, variants: readonly VariantInfo[]) => R;
  brand?: (tag: string, inner: TypeObject) => R;
  readonly?: (inner: TypeObject) => R;
  refinement?: (refinements: readonly RefinementInfo[], inner: TypeObject) => R;
  result?: (valueType: TypeObject | null, errorType: TypeObject | null) => R;
  option?: (valueType: TypeObject | null) => R;
  metadata?: (metadata: { name?: string; source?: string }, inner: TypeObject) => R;
  port?: (portName: string, methods: readonly PortMethodInfo[]) => R;
  effect?: (effectType: string, returnType: TypeObject) => R;
}
 ...
 * ```
 */
export function hashSchema(schema: TypeObject): string {
