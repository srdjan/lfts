...
function collectErrors(
  ...
  patternMatch<number, void>(op)
    ...
    .with(Op.REFINE_POSITIVE, () => {
      ...
      if (
        errors.length < maxErrors && typeof value === "number" && value <= 0
      ) {
        errors.push({
          path: buildPath(pathSegments),
    ...
    .with(Op.REFINE_NON_EMPTY, () => {
      ...
      if (
        errors.length < maxErrors && Array.isArray(value) && value.length === 0
      ) {
        errors.push({
          path: buildPath(pathSegments),
  ...
  for (let i = 0; i < methodCount; i++) {
    if (portSchema[offset] !== Op.PORT_METHOD) {
      return Result.err({
        type: "not_object",
        message:
          `Invalid PORT bytecode: expected PORT_METHOD at offset ${offset}`,
      });
    }
    ...
    if (method === undefined) {
      return Result.err({
        ...
        methodName,
        message:
          `Port ${portName} requires method '${methodName}', but it is missing`,
      });
    }
