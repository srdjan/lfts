// packages/lfts-type-compiler/src/gate/gate.ts
...
export function runGate(
  ...
  const normalizedSrcDir = (srcDir.endsWith("/") ? srcDir : srcDir + "/")
    .replace(/\/+/g, "/");
  return program
    .getSourceFiles()
    .filter((sf) =>
      !sf.isDeclarationFile && sf.fileName.startsWith(normalizedSrcDir)
    )
  ...
  const currentDiag = isBannedSyntax(node)
    ? [
      createDiag(
        sf,
        node,
        `LFP000x: Disallowed syntax in Iteration-1: ${
