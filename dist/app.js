import { AsyncResult, match, validate, validateSafe, } from "../../packages/lfts-type-runtime/mod.ts";
...
import { decodeText } from "./util.ts";
...
function toTaskList(store: Store): {
    tasks: readonly Task[];
} {
...
async function persistStore(store: Store, storage: StoragePort, io: ConsolePort): Promise<void> {
    const json = JSON.stringify(validate(TaskList$, toTaskList(store)), null, 2);
...
async function loadStore(storage: StoragePort, io: ConsolePort): Promise<Store> {
...
export async function handleCommand(store: Store, clock: ClockPort, io: ConsolePort, storage: StoragePort, cmd: Command): Promise<Store> {
    return match(cmd, {
        add: async (c) => {
